    ...
    pub fn update(&self, block: &Block) {
        ...
        for tx in block.get_transactions() {
            if tx.is_coinbase() == false {
                for vin in tx.get_vin() { // remove / update spent outputs of given transaction from UTXO set
                    ...
                    for (idx, out) in outs.iter().enumerate() {
                        if idx != vin.get_vout() {
                            updated_outs.push(out.clone()) // still unspent output
                        }
                    }
                    if updated_outs.len() == 0 { // all outputs are spent
                        let _ = utxo_tree.remove(vin.get_txid()).unwrap(); // remove the key
                    } else {
                        let outs_bytes = bincode::serialize(&updated_outs)
                            .expect("unable to serialize TXOutput");
                        utxo_tree.insert(vin.get_txid(), outs_bytes).unwrap(); // update the value
                    }
                }
            }
            let mut new_outputs = vec![];
            for out in tx.get_vout() { // add outputs of new transaction as it is
